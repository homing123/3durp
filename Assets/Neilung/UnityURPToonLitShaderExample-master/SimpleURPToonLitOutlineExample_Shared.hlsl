...
Varyings VertexShaderWork(Attributes input)
{
    ...
    //outline 위치 = posWS + normalWS * 아웃라인 크기(카메라정보와 _OutlineWidth 프로퍼티로 구함)
    ...
    //아웃라인을 그리는 경우엔 월드위치가 변하므로 다시 계산해서 넣어줌
    ...
    //_OutlineZOffset(_OutlineZOffset * outlineZOffsetMask + 0.03 * _IsFace) 값 만큼 VS좌표계에서 이동 후 positionCS로 다시 수정 (Z값만 수정함)
