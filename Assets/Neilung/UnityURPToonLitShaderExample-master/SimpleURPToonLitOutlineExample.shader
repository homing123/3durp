...
because:
    ...
    - we want to avoid shader size and memory usage becomes too large easily (2^n), 2GB memory iOS mobile will crash easily if you use too much multi_compile //멀티컴파일 많이쓰면 메모리 터지니까 적게쓰겠다.
    ...
    - we want to avoid breaking SRP batching because SRP batching is per shader variant batching, not per shader //셰이더 멀티컴파일벼로 배칭되기때문에 컴파일분기를 줄이겠다.
    ...
    case 1 - compile time constant if(): //무조건적으로 안쓰이는 if는 알아서 제거해준다.
    ...
    case 3 - dynamic branching if() without divergence inside a wavefront/warp:  //동적분기가 픽셀 8x8그룹이 모두 포함된다면 warp분기가 발생하지 않는다. 근데 8x8의 시작기준이 다르지않나?
